...
const createIsland = async (req, res) => {
  const cid = req.user.company._id;
  const { name, branchId, branchName, pumps, productTypes, includesGas, rotationOrder, maxWorkers, status, notes } = req.body;
...
const updateIsland = async (req, res) => {
  ...
  const fields = ['name', 'pumps', 'productTypes', 'includesGas', 'rotationOrder', 'maxWorkers', 'status', 'notes'];
